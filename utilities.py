...
def send_payload_elastic(payload):
    es = Elasticsearch(["localhost"], port=9200)
    es.index(index='fileevents', body=payload)


def check_and_insert_database(payload, dbfile, api_key):
    conn, cursor = database_init(dbfile)
...
def event_handler(event):
    ...
    if not payload['isDirectory']:
        ...
        settings = read_yaml()
        check_and_insert_database(payload, settings['databaseFile'], settings['virustotalAPI'])
    else:
        send_payload_elastic(payload)
